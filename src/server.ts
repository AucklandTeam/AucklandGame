...
app.post(ApiLocation.FORUM, async (req, res) => {
	if (req.body?.label) {
		const category = await CategoryTopicService.createCategory(
			req.body.label
		)
		res.send(category)
	}
...
app.get(
	ApiLocation.TOPIC,
	async (
		req: Request<undefined, undefined, undefined, { topicId: number }>,
		res: Response
	) => {
...
app.post(
	ApiLocation.COMMENT,
	async (req: Request<IComment, IComment, IComment>, res: Response) => {
