...
const CanvasComponent: FC<CanvasProps> = ({
	...
	const fireShip = () => {
		...
		if (isTripleFire) {
			const x1 =
				...
				shipWith / 2 -
				bulletRadius / 2 +
				50 * Math.sin(((-angle + 50) * Math.PI) / 180)
			const y1 =
				yMove -
				bulletRadius / 2 +
				shipHeight / 2 +
				50 * Math.cos(((-angle + 50) * Math.PI) / 180)
			const x2 =
				xMove +
				shipWith / 2 -
				bulletRadius / 2 +
	...
	const checkCollision = () => {
		...
		bombs.forEach((bomb: any) => {
			if (
				Math.abs(xMove + 50 - bomb.x - 30) < 50 &&
				Math.abs(yMove + 50 - bomb.y - 30) < 50
			) {
				bomb.isVisible = false
				const bombExpObj = new Sprite(
					bomb.x + 20,
					bomb.y + 20,
					810,
					9,
		...
		asteroids.forEach((asteroid: any) => {
			bullets.forEach((bullet: any) => {
				if (
					Math.abs(bullet.getPos().x + 1 - asteroid.getCenterX()) <
						asteroid.radius &&
					Math.abs(bullet.getPos().y + 1 - asteroid.getCenterY()) <
						asteroid.radius
				) {
	...
	const getAsteroidCoords = (): any => {
		...
		const x: number =
			getRandomArbitrary(0, canvas.width) + canvas.width * offset
		const y: number =
			getRandomArbitrary(0, canvas.height) + canvas.height * offset
		return { x, y }
	}
