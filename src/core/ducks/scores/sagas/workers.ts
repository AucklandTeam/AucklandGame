...
import { addUserToLeaderBoardAction, seLeaderBoardData, setLeaderBoardStatus, setLeaderBoardFailed } from '../actions'
...
export function* getLeaderBoardWorker(): SagaIterator<void> {
    ...
    try {
        const response: ResponseLeaderBoardList = yield call(getAllLeaderBoard, {
            cursor: 0,
            limit: 10,
...
export function* addUserToLeaderBoardWorker({
    payload,
}: ReturnType<typeof addUserToLeaderBoardAction>): SagaIterator<void> {
    try {
